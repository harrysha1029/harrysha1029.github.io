...
[^implies]: Sometimes this axiom will have \\(\implies\\) in place of the second \\(\iff\\). But we get the other direction from the substitution property of equality see [here](https://en.wikipedia.org/wiki/Axiom_of_extensionality#Formal_statement) for more info. 
...
[^continues]: It actually keeps going! But to talk about that we need to go into more details about ordinals. 
